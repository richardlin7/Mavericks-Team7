...
public class AdminController {
	...
	// Inserting new Admin
	@PostMapping("/admin/{lId}/{cId}")
	public Admin registerAdmin(@RequestBody Admin admin, @PathVariable("lId") Long lId,@PathVariable("cId") Long cId) {
		...
		Optional<LoginCredentials> opt1 = loginCredentialsRepository.findById(cId);
		if (!opt1.isPresent()) {
			throw new RuntimeException("Login Credential ID Invalid");
		}
		LoginCredentials loginCredentials = opt1.get();
